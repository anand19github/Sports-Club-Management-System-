...
public class BadmintonPage {
    public static void main(String[] args) {
        JFrame frame = new JFrame("Badminton Registration");
        frame.setSize(800, 600);
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

        JLabel background = new JLabel(new ImageIcon("images/badminton.jpg"));
        background.setLayout(new BorderLayout());
        frame.setContentPane(background);

        JLabel title = new JLabel("Badminton Registration", SwingConstants.CENTER);
        ...
        registerButton.addActionListener(e -> {
            ...
            try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/sportsclub", "root", "");
                ...
                PdfWriter.getInstance(document, new FileOutputStream("Badminton_Registration_" + name + ".pdf"));
                document.open();
                document.add(new Paragraph(
                        "Thank you " + name + " for registering for Badminton!\nWe’ll contact you at " + email));
                document.close();
